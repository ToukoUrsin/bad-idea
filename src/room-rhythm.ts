...
export const roomMemorySchema=z.object({
 ...
 challengeMode:z.enum(['geometry','rules','mixed']).optional(),inventoryReset:z.boolean().optional(),
...
type RememberedRoom=Pick<RoomMemory,'round'|'title'|'change'|'challengeMode'|'objective'|'gameplay'|'inventoryReset'> & {rule?:{restriction:string,language?:LanguageRule}};
...
export function rememberRoom(room:RememberedRoom):RoomMemory{
 ...
 return roomMemorySchema.parse({...room,inventoryReset:room.inventoryReset===true,restriction:room.rule?.restriction,languageRule:language?languageRuleText(language):undefined});
}

...
import {firstArena,ruleText,type Arena} from './arena';
...
<div id="pause" class="overlay hidden"><div class="pause-box"><span class="micro">PAUSED</span><h2>Take a moment.</h2><button id="resume">Resume ↗</button><button id="pause-invent">Invent <kbd>E</kbd></button><button id="restart">Retry round <kbd>R</kbd></button><button id="level-menu">Choose a room</button><button id="new-run">New adaptive run</button><div class="settings"><label for="sensitivity">LOOK SENSITIVITY <output id="sensitivity-value">1.0</output></label><input id="sensitivity" type="range" min="0.3" max="2" step="0.1" value="1"><label class="check"><input id="motion" type="checkbox"> CAMERA MOTION</label><label class="check"><input id="sound" type="checkbox" checked> SOUND</label><p>WASD move · Shift sprint · Space jump<br>R retry · Q / C turn<br>E invent · F use · H help · Esc back<br>Mouse looks · F uses · I opens inventory<br>Enter builds · Shift+Enter adds a line</p></div></div></div>
<div id="result" class="overlay hidden"><div class="result-box"><span class="micro" id="result-tag"></span><h2 id="result-title"></h2><p id="result-copy"></p><div id="counter-card" class="hidden"><span class="micro">THE ROOM RESPONDS</span><h3 id="counter-title"></h3><p id="counter-change"></p><p id="counter-hint"></p><p id="counter-rule"></p></div><p>Enter Continue · I Inventory · E Invent · R Retry</p><p id="director-error" class="hidden"></p><button id="retry-director" class="hidden">Try generating again</button><button id="retry">TRY AGAIN</button><button id="next-level" class="hidden">NEXT LEVEL ↗</button></div></div>
...
let arena:Arena|null={...firstArena},nextArena:Arena|null=null,roomHistory:{round:number,title:string,change:string,challengeMode?:string}[]=[],nextFor='',directorBusy=false,directorError='',directorController:AbortController|null=null,directorVersion=0,roomFailure='',previousForRule:Invention|null=null;
let plannedBuild:{key:string,actualKey?:string}|null=null;
let runVersion=0;
let escapeHandledUntil=0;let inventorySelection=0;let inventoryOpen=false;let inventory:{key:string,item:Invention}[]=[];
...
function showCounter(){
 const available=!!nextArena,eligible=sim.state==='won'||!!roomFailure;
 $('#next-level').classList.toggle('hidden',!eligible);$<HTMLButtonElement>('#next-level').disabled=!available;
 $('#next-level').textContent=available?(roomFailure?'Enter repaired room ↗':'Enter round '+nextArena!.round+' ↗'):'Astra is changing the room…';
 $('#counter-card').classList.toggle('hidden',!eligible||!available);
 if(nextArena){$('#counter-title').textContent=nextArena.title;$('#counter-change').textContent=nextArena.change;$('#counter-hint').textContent=nextArena.hint;$('#counter-rule').textContent=nextArena.rule.restriction?ruleText(nextArena.rule)+(nextArena.carryAllowed?'':' '+nextArena.carryReason):'';}
 $('#director-error').textContent=directorError;$('#director-error').classList.toggle('hidden',!eligible||!directorError);$('#retry-director').classList.toggle('hidden',!eligible||!directorError);
}
async function prepareNext(repair?:string,planned?:{prompt:string,previous:Invention|null}){
 ...
 try{
  const r=await fetch('/api/direct',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({arena:currentArena,previous:planned?.previous||spec,plannedInvention:planned,repair,history:roomHistory,inventory:inventoryPayload(),telemetry:{won:sim.state==='won',elapsed:sim.elapsed,uses:sim.uses,maxHeight:sim.maxHeight,position:sim.position,removed:sim.entities.filter(e=>e.removed).map(e=>e.id),events:sim.events.slice(-8)}}),signal:directorController.signal});
  const data=await r.json();if(!r.ok)throw Error(data.error||'The next room could not be built.');if(version!==directorVersion)return;
...
function attachCode(){
 if(!spec?.code)return;
 new CodeRuntime(sim,view,message=>queueMicrotask(()=>{if(busy)return;craft();showCard();void buildInvention('Repair this invention so its intended behavior works.',spec,message);}),(...args)=>sound.tone(args[0],args[1],args[2],args[3]),spec.code,'invention',true);
}
function saveIdeas(){try{localStorage.setItem('badidea-inventions-v1',JSON.stringify({spec,draft,level}));localStorage.setItem('badidea-run-v1',JSON.stringify({arena,nextArena,roomHistory,nextFor,spec,draft,previousForRule,plannedBuild}));}catch{/* Storage may be unavailable; gameplay still works. */}}
...
for(const [i,b] of [...document.querySelectorAll<HTMLButtonElement>('[data-prompt]')].entries()){b.dataset.prompt=ideas[i];b.textContent=ideas[i];}startCounter();}
...
function equipSample(){if(arena&&arena.round>1){craft();return;}draft=sample();$('#equip').click();}
...
function craft(){if(!ready)return;inventoryOpen=false;if(!hasStarted){reset();hasStarted=true;}edgeTurn=0;workbenchOpen=true;showMode('play');showCard();$<HTMLTextAreaElement>('#prompt').focus();}
$('#enter').onclick=play;$('#resume').onclick=play;$('#close-workshop').onclick=play;$('#equip').onclick=()=>equipDraft(true);$('#pause-invent').onclick=craft;
$('#restart').onclick=()=>{reset();play();};$('#retry').onclick=()=>{reset();play();};
function inventoryPayload(){return inventory.map(({key,item})=>({key,name:item.name,description:item.description,usage:item.usage,limitations:item.limitations,code:item.code}));}
function legality(item:Invention){if(!arena?.rule.restriction)return null;return arena.inventoryVerdicts?.find(v=>v.key===inventionKey(item))||{key:inventionKey(item),allowed:false,reason:'Checking this round’s rule…'};}
...
async function checkInventoryRules(){
 ...
 ruleCheck=(async()=>{try{const response=await fetch('/api/check-rule',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({arena:current,inventory:missing})});const data=await response.json();if(!response.ok)throw Error(data.error);if(arena!==current)return;current.inventoryVerdicts=[...(current.inventoryVerdicts||[]),...data.verdicts];saveIdeas();drawInventory();}catch{say('Rule check needs another try. Reopen inventory.');}finally{ruleCheck=null;}})();await ruleCheck;
}
function remember(item:Invention){
 const key=inventionKey(item);if(!inventory.some(entry=>entry.key===key))inventory.unshift({key,item});
...
function equipDraft(resume:boolean){
 if(draft){
  ...
  spec=draft;draft=null;previousForRule=null;remember(spec);sim.equip(spec);view.setInvention(spec);attachCode();if(spec.autoDeploy)sim.activate();
...
function drawInventory(){
 ...
 for(const [index,entry] of inventory.entries()){const button=document.createElement('button');button.className='inventory-item';const name=document.createElement('span');name.textContent=(index<9?(index+1)+'. ':'')+entry.item.name;const state=document.createElement('small');const verdict=legality(entry.item);state.textContent=verdict&&!verdict.allowed?(verdict.reason==='Checking this round’s rule…'?'Checking…':'Not allowed'):inventionKey(spec)===entry.key?'Equipped':'Equip ↗';button.title=verdict?.reason||'';button.disabled=!!verdict&&!verdict.allowed;button.append(name,state);button.classList.toggle('selected',inventionKey(spec)===entry.key);button.classList.toggle('highlighted',inventorySelection===index);button.setAttribute('aria-selected',String(inventorySelection===index));button.onclick=()=>{draft=entry.item;equipDraft(true);};list.append(button);}
}
...
function drawChapters(){
 ...
 for(const b of document.querySelectorAll<HTMLButtonElement>('[data-level]'))b.onclick=()=>{runVersion++;controller?.abort();invalidateDirector();arena=null;roomFailure='';level=Number(b.dataset.level) as LevelId;spec=null;draft=null;saveIdeas();reset();hasStarted=true;play();};
}
drawChapters();
$('#level-menu').onclick=()=>showMode('intro');
$('#new-run').onclick=()=>{runVersion++;controller?.abort();invalidateDirector();arena={...firstArena};roomHistory=[];roomFailure='';level=0;spec=null;draft=null;previousForRule=null;saveIdeas();reset();hasStarted=true;play();};
$('#next-level').onclick=()=>{if(arena){if(!nextArena)return;const old=arena;arena=nextArena;nextArena=null;$('#director-status').textContent='';roomHistory=[...roomHistory,{round:old.round,title:old.title,change:old.change,challengeMode:old.challengeMode}].slice(-6);roomFailure='';nextFor='';plannedBuild=null;if(spec&&legality(spec)?.allowed===false){remember(spec);previousForRule=spec;spec=null;draft=null;}saveIdeas();reset();void checkInventoryRules();play();return;}level=Math.min(2,level+1) as LevelId;spec=null;draft=null;saveIdeas();reset();play();};
...
function showCard(){
 ...
 $('#invent').innerHTML=item?'Modify <span>↗</span>':'Build <span>↗</span>';
 if(!item)return;
...
$('#new-idea').onclick=()=>{if(busy)return;startingNew=true;draft=null;$<HTMLTextAreaElement>('#prompt').value='';$<HTMLTextAreaElement>('#prompt').placeholder='A ladder, an ice ray, an invisible cloak…';showCard();$<HTMLTextAreaElement>('#prompt').focus();};
for(const button of document.querySelectorAll<HTMLButtonElement>('[data-prompt]'))button.onclick=()=>{if(busy)return;startingNew=true;draft=null;showCard();$<HTMLTextAreaElement>('#prompt').value=button.dataset.prompt!;$<HTMLTextAreaElement>('#prompt').focus();};
async function buildInvention(prompt:string,previous:Invention|null,repair?:string){
 if(busy||!ready)return;if(arena&&!repair&&!directorBusy&&!nextArena){plannedBuild={key:'prompt:'+Date.now()+':'+prompt};void prepareNext(undefined,{prompt,previous});}busy=true;controller=new AbortController();$('#invent').setAttribute('disabled','');$('#equip').setAttribute('disabled','');$('#starter').setAttribute('disabled','');$('#generation').classList.remove('hidden');$('#error').classList.add('hidden');
 const startTime=performance.now();const buildRun=runVersion,buildArena=arena;let stage='BUILDING';const ticker=setInterval(()=>{const seconds=((performance.now()-startTime)/1000).toFixed(1);$('#timer').textContent=`${seconds}s · ${stage}`;$('#build-notice').textContent=`${repair?'Repairing':'Building'} · ${seconds}s · Keep playing`;},100);workbenchOpen=false;showMode('play');$<HTMLTextAreaElement>('#prompt').blur();void captureMouse();$('#build-notice').classList.remove('hidden');
 try{
  stage=repair?'REPAIRING':'BUILDING';
  const r=await fetch('/api/invent',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({prompt,previous,repair,level,arena,scene:{player:{position:[sim.position.x,sim.position.y,sim.position.z],mass:1,walkSpeed:3.2},guard:level===0&&!arena?{position:[sim.guard.x,0,sim.guard.z]}:null,keyOwner:arena?'none':level===0?sim.keyOwner:'none',gateOpen:sim.gateOpen}}),signal:controller.signal});
  const data=await r.json();if(!r.ok)throw Error(data.error||'Astra could not finish that invention.');
  if(runVersion!==buildRun)return;draft=prepareInvention(data.invention);remember(draft);if(arena?.rule.restriction){if(arena===buildArena){arena.inventoryVerdicts=[...(arena.inventoryVerdicts||[]).filter(v=>v.key!==inventionKey(draft)),{key:inventionKey(draft),allowed:true,reason:''}];}else await checkInventoryRules();}if(plannedBuild){plannedBuild.actualKey=inventionKey(draft);if(nextFor===plannedBuild.key)nextFor=plannedBuild.actualKey;}startingNew=false;saveIdeas();showCard();$('#speed').textContent=`Built in ${((performance.now()-startTime)/1000).toFixed(1)}s`;
  $<HTMLTextAreaElement>('#prompt').value='';$<HTMLTextAreaElement>('#prompt').placeholder='Describe a change…';const equippedName=draft.name;equipDraft(false);say(draft?'Saved to inventory. Check this round’s rule.':equippedName+' equipped. I for inventory.');

 }catch(e){if((e as Error).name!=='AbortError'){$('#error').textContent=(e as Error).message;$('#error').classList.remove('hidden');$('#build-notice').textContent='Build needs another try · Open';}}
 finally{clearInterval(ticker);busy=false;controller=null;$('#generation').classList.add('hidden');$('#invent').removeAttribute('disabled');$('#equip').removeAttribute('disabled');$('#starter').removeAttribute('disabled');if(!draft&&$<HTMLElement>('#error').classList.contains('hidden'))$('#build-notice').classList.add('hidden');}
}
...
$('#build-notice').onclick=()=>{if(draft&&!busy)$('#equip').click();else craft();};
...
window.addEventListener('keydown',e=>{
 ...
 if(inventoryOpen){
  ...
  if(slot>=0&&inventory[slot]){e.preventDefault();draft=inventory[slot].item;equipDraft(true);return;}
  return;
 ...
 if(k==='enter'&&mode==='result'){e.preventDefault();const next=$<HTMLButtonElement>('#next-level');if(!next.classList.contains('hidden')&&!next.disabled)next.click();else if(sim.state!=='won')$('#retry').click();return;}
 if(k==='enter'&&(mode==='pause'||mode==='intro')){e.preventDefault();void play();return;}
...
window.addEventListener('blur',()=>{keys.clear();if(mode==='play')showMode('pause');});
...
try{await initPhysics();sim=new Simulation();view=new GameView($('#stage'));view.renderer.domElement.tabIndex=-1;preview=new InventionPreview($('#preview'));try{const saved=JSON.parse(localStorage.getItem('badidea-inventions-v1')||'null');if(saved){level=([0,1,2].includes(saved.level)?saved.level:0) as LevelId;spec=saved.spec?prepareInvention(saved.spec):null;draft=saved.draft?prepareInvention(saved.draft):null;}}catch{/* Ignore invalid saved drafts. */}try{const run=JSON.parse(localStorage.getItem('badidea-run-v1')||'null');if(run){arena=run.arena;nextArena=run.nextArena;roomHistory=run.roomHistory||[];nextFor=run.nextFor||'';plannedBuild=run.plannedBuild||null;if(plannedBuild&&!plannedBuild.actualKey)plannedBuild=null;previousForRule=run.previousForRule||null;spec=run.spec?prepareInvention(run.spec):null;draft=run.draft?prepareInvention(run.draft):null;if(arena)level=0;}else{arena={...firstArena};spec=null;draft=null;level=0;}}catch{arena={...firstArena};spec=null;draft=null;level=0;}try{inventory=JSON.parse(localStorage.getItem('badidea-inventory-v1')||'[]').map((entry:{item:unknown})=>{const item=prepareInvention(entry.item);return {key:inventionKey(item),item};});}catch{inventory=[];}if(spec)remember(spec);sim.dispose();sim=new Simulation(level,arena);if(arena)view.setArena(arena);else view.setLevel(level);ready=true;view.yaw=-Math.PI/2;view.pitch=-.06;let last=performance.now(),acc=0,eventCount=0;
 function frame(now:number){const delta=Math.min((now-last)/1000,.08);last=now;if(mode==='play'){if(fallbackLook&&!document.pointerLockElement&&!workbenchOpen&&!inventoryOpen)view.yaw-=Math.max(-1,Math.min(1,edgeTurn))*2.2*delta;sim.aim={x:-Math.sin(view.yaw)*Math.cos(view.pitch),y:Math.sin(view.pitch),z:-Math.cos(view.yaw)*Math.cos(view.pitch)};if(keys.has('q'))view.yaw+=2*delta;if(keys.has('c'))view.yaw-=2*delta;acc+=delta;let forward=0,side=0;if(keys.has('w')||keys.has('arrowup'))forward++;if(keys.has('s')||keys.has('arrowdown'))forward--;if(keys.has('a')||keys.has('arrowleft'))side--;if(keys.has('d')||keys.has('arrowright'))side++;const length=Math.hypot(forward,side)||1,speed=keys.has('shift')?4.5:3.2;sim.manual={x:(-Math.sin(view.yaw)*forward+Math.cos(view.yaw)*side)/length*speed,y:0,z:(-Math.cos(view.yaw)*forward-Math.sin(view.yaw)*side)/length*speed};while(acc>=1/60){sim.tick();if(sim.landingSpeed>1)sound.land();sound.tick(Math.hypot(sim.player.linvel().x,sim.player.linvel().z),sim.grounded,1/60);acc-=1/60;}
  ...
  if(arena)hint=sim.disrupted>0?'Recovering · Keep moving':arena.rule.restriction?arena.rule.title:'';$('#interaction').textContent=hint;
  ...
  if(sim.state==='won'||sim.state==='lost'){
   workbenchOpen=false;inventoryOpen=false;const won=sim.state==='won';showMode('result');$('#result').classList.toggle('won',won);
   $('#result-tag').textContent=arena?`ROUND ${arena.round}`:levels[level].name;
   $('#result-title').textContent=won?'You got out.':roomFailure?'A room error.':'Try another angle.';
   $('#result-copy').textContent=won?(arena?'The room has been watching.':level<2?'The next obstacle is waiting.':'All three rooms escaped.'):sim.reason;
   $('#retry').textContent=won?'Replay round · R':'Retry · Enter';
   if(arena){if(won&&!nextArena&&!directorBusy&&!directorError)void prepareNext();showCounter();}
   else{$('#counter-card').classList.add('hidden');$('#next-level').classList.toggle('hidden',!won||level===2);$('#next-level').textContent='Next level ↗';$<HTMLButtonElement>('#next-level').disabled=false;}
 ...
 $('#bubble').classList.toggle('hidden',now>noticeUntil||mode!=='play');view.render(sim,delta,mode==='play'?acc*60:1,mode==='play');if(workbenchOpen&&mode==='play')preview.render();requestAnimationFrame(frame);
 }
